    ...
    return (
        <div className="min-h-screen bg-[#F9F1E3] flex items-center justify-center px-4 py-12">
            <div className="w-full max-w-4xl">
                <h1 className="text-3xl font-bold text-center text-[#4B3621] mb-8">💉 Control de Vacunas</h1>

                {showForm ? (
                    <form onSubmit={registrarVacuna} className="bg-white p-6 rounded-2xl shadow-md space-y-4 mb-8">
                        <h2 className="text-xl font-semibold text-[#4B3621] text-center">Registrar Vacuna</h2>
                        <select
                            name="mascotaId"
                ...
                <div className="bg-white rounded-2xl shadow-md p-6">
                    <h2 className="text-xl font-semibold text-[#4B3621] mb-4">Historial de Vacunas</h2>
                ...
                {editData && (
                    <div className="fixed inset-0 bg-[#4B3621]/20 backdrop-blur-sm flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md">
                            <h2 className="text-xl font-semibold text-center text-[#4B3621] mb-4">Editar Vacuna</h2>
                            <form onSubmit={handleUpdate} className="space-y-4">
                                <select
                                    name="mascotaId"
                                    value={editData.mascotaId}
                                    onChange={(e) => setEditData({ ...editData, mascotaId: e.target.value })}
                                    className="w-full border border-gray-300 text-gray-700 p-3 rounded-xl"
                                    required
                                >
                                    {mascotas.map((m) => (
                                ...
                                />
                                <div className="flex justify-between">
                                    <button type="submit" className="bg-[#B08968] text-white px-4 py-2 rounded-xl hover:bg-[#936E4E]">Guardar</button>
                                    <button type="button" onClick={() => setEditData(null)} className="text-[#C24E4E] hover:text-red-700">Cancelar</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                {deleteId && (
                    <div className="fixed inset-0 bg-[#4B3621]/20 backdrop-blur-sm flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-sm text-center">
                            <h2 className="text-xl font-semibold text-[#4B3621] mb-4">¿Eliminar esta vacuna?</h2>
